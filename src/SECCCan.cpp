...
void EVSEModelCan::CanProtocol_AuthorizeReq(Payload_AuthorizeReq& payload){
    ...
    idtag = IDTag;
    UsingconnecterID = payload.connectorID;
    /*
    if (idtag!=NULL)
    {
    Serial.print(" IDTag: ");
...
void EVSEModelCan::CanProtocol_MeterValueNtf(Payload_MeterValueNtf& payload){
    ...
    if (CanPacketSave[CanID- frameID])
        {
            strcpy(SavedPacket,CanPacketSave[CanID- frameID].c_str());
            payload.connectorID = (int)SavedPacket[0];
            payload.L1Voltage = (float)SavedPacket[1]*100+(float)SavedPacket[2]+(float)SavedPacket[3];
            payload.L1Current = (float)SavedPacket[4]*100+(float)SavedPacket[5]+(float)SavedPacket[6];
        }
    if (CanPacketSave[CanID- frameID*2])
        {
            strcpy(SavedPacket,CanPacketSave[CanID- frameID*2].c_str());
            payload.L1Power = (float)SavedPacket[0]*100+(float)SavedPacket[1]+(float)SavedPacket[2];
            payload.L2Voltage = (float)SavedPacket[3]*100+(float)SavedPacket[4]+(float)SavedPacket[5];
        }    
    if (CanPacketSave[CanID- frameID*3])
        {
            strcpy(SavedPacket,CanPacketSave[CanID- frameID*3].c_str());
            payload.L2Current = (float)SavedPacket[0]*100+(float)SavedPacket[1]+(float)SavedPacket[2];
            payload.L2Power = (float)SavedPacket[3]*100+(float)SavedPacket[4]+(float)SavedPacket[5];
        }        
    if (CanPacketSave[CanID- frameID*4])
        {
            strcpy(SavedPacket,CanPacketSave[CanID- frameID*4].c_str());
            payload.L3Voltage = (float)SavedPacket[0]*100+(float)SavedPacket[1]+(float)SavedPacket[2];
            payload.L3Current = (float)SavedPacket[3]*100+(float)SavedPacket[4]+(float)SavedPacket[5];
        }    
    if (CanPacketSave[CanID- frameID*5])
        {
            strcpy(SavedPacket,CanPacketSave[CanID- frameID*5].c_str());
            payload.L3Power = (float)SavedPacket[0]*100+(float)SavedPacket[1]+(float)SavedPacket[2];
        }    
    ...
    //Serial.println((String)payload.currentTime);
    Serial.println(UsingconnecterID);
